...
def train_linear_learner(rate, seed):
    input_file = open('training_data.txt')
    ...
    for iteration in list(range(0, num_iterations)):
        for entryNumber in list(range(0, len(asteroid_chunk_mass_list))):
...
def evaluate_learner(learner_weights):
    input_file = open('testing_data.txt')
    asteroid_chunk_mass_list = list()
    asteroid_chunk_selling_price_list = list()
    error = 0
    ...
    for entryNumber in list(range(0, len(asteroid_chunk_mass_list))):
        mass = float(asteroid_chunk_mass_list[entryNumber])
        actual_selling_price = float(asteroid_chunk_selling_price_list[entryNumber])
